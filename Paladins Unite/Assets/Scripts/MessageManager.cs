    ...
    void Update()
    {
        if (SettingsManager.playing){
            if (cooldownCounter < 0){
                ...
                {
                    if (Vector3.Distance(npc.transform.position, player.transform.position) < SettingsManager.interactableDist){
            ...
            if (Input.GetKeyDown(SettingsManager.keyLabels["selectButton"])){
                ...
                if (messageBox.text == lines[index]){
                    NextLine();
                    currentTextSpeed = SettingsManager.defaultTextSpeed/10;
                } else {
                    currentTextSpeed = SettingsManager.defaultTextSpeed/10;
                }
            }
    ...
    IEnumerator TypeLine()
    {
        ...
        foreach (char c in lines[index].ToCharArray()){
            ...
            if (Char.ToLower(c) == 'a' || Char.ToLower(c) == 'e' || Char.ToLower(c) == 'i' || Char.ToLower(c) == 'o' || Char.ToLower(c) == 'u') {
                audioPlayer.PlayOneShot(audio1);
            } else {
                audioPlayer.PlayOneShot(audio2);
            }
            i ++;
            if (closing && c == '>'){
                closing = false;
