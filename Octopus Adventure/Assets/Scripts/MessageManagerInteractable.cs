...
public class MessageManagerInteractable : MonoBehaviour
{
    ...
    void Update()
    {
        if (SMR.playing){
            if (cooldownCounter < 0){
                GameObject[] npcs = GameObject.FindGameObjectsWithTag("InteractableObj");
                bool closeBy = false;
                foreach (GameObject npc in npcs)
                {
                    if (Vector3.Distance(npc.transform.position, player.transform.position) < SMR.interactableDist){
                        interactable.transform.position = player.transform.position + new Vector3(0, 1.5f, 0);
                        closeBy = true;
                        break;
                    }
                }
